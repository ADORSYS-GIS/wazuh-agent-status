...
- **The Brains (Server)**: A background service that runs with "High Privilege." It stays hidden and handles the heavy lifting—talking to the OS, managing service states, and interacting with the Wazuh Agent's files.
...
The current Go implementation uses a simple TCP polling model. For a detailed technical breakdown of the performance and security limitations of the current system, see:
