...
**The `wazuh-gateway` becomes the compliance evaluation and aggregation layer between Wazuh native SCA data and compliance consumers. Compliance evaluation belongs to the gateway, not to individual consumers.**
...
The authoritative inputs are Wazuh SCA results and corporate compliance profiles. The Compliance Model generated by the gateway is the authoritative representation consumed by downstream applications. OpenSearch is a projection, not the source of truth.
